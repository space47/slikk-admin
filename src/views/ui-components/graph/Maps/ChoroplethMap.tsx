...
const colorScale = scaleLinear<string>().domain([0.29, 0.68]).range(['#ffedea', '#ff5233'])
                ...
                <Geographies geography={geoUrl}>
                    {({ geographies }) =>
                        geographies.map((geo) => {
                            ...
                            return <Geography key={geo.rsmKey} geography={geo} fill={d ? colorScale(Number(d['2017'])) : '#F5F4F6'} />
                        })
                    }
                </Geographies>
            )}
