...
import SalesReportLineGraph from './SalesReportLineGraph'
import SalesReportPieGraph from './SalesReportPieChart'
import SalesReportCompositeGraph from './SalesReportCompositeGraph'
            ...
            {(graphType || graphType === undefined || graphType === '') && selectedOption && (
                <>
                    {selectedOption === 'line' && (graphType === 'line' || graphType === undefined || graphType === '') && (
                        <SalesReportLineGraph xAxisData={xAxisData} yAxisData={yAxisData} type="line" />
                    )}
                    {selectedOption === 'bar' && (graphType === 'bar' || graphType === undefined || graphType === '') && (
                        <SalesReportLineGraph xAxisData={xAxisData} yAxisData={yAxisData} type="bar" />
                    )}
                    {selectedOption === 'pie' && (graphType === 'pie' || graphType === undefined || graphType === '') && (
                        <SalesReportPieGraph xAxisData={xAxisData} yAxisData={yAxisData} />
                    )}
                    {selectedOption === 'heatmap' && (graphType === 'heatmap' || graphType === undefined || graphType === '') && (
                        <SalesReportLineGraph xAxisData={xAxisData} yAxisData={yAxisData} type="heatmap" />
                    )}
                    {selectedOption === 'composite' && (graphType === 'composite' || graphType === undefined || graphType === '') && (
                        <SalesReportCompositeGraph xAxisData={xAxisData} yAxisData1={yAxisData} yAxisData2={yAxisData2} />
                    )}
                </>
            )}
        </div>
