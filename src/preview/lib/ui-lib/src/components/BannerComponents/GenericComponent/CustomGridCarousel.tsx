...
export function CustomGridCarousel<T>({
    ...
    function distributeItemsInGrid<T>({ items, rows, columns }: { items: T[]; rows: number; columns: number }): T[][] {
    ...
    const cols = size == 'lg' ? component_config.web_column || 3 : component_config.column || 3
    ...
    const grid = distributeItemsInGrid({
        ...
        columns: size == 'lg' ? component_config.web_column || 3 : component_config.column || 3,
    })
    ...
    return (
        <div className="w-full mx-auto relative">
            <div className={`overflow-hidden ${!autoplayEnabled ? 'cursor-grab active:cursor-grabbing' : ''}`} ref={emblaRef}>
            ...
            {showDots && scrollSnaps?.length > 1 && (
                <div className="flex justify-center mt-4 cursor-pointer">
                    {scrollSnaps.map((_, index) => (
                        <button
                            key={index}
                            className={`h-2 rounded-full mx-1 p-0 min-w-0 ${index === selectedIndex ? 'w-10 bg-white' : 'w-2 bg-white/50'}`}
