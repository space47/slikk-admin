...
import { HiOutlinePencil, HiOutlineTrash, HiXCircle, HiOutlineExclamation, HiOutlinePlusCircle, HiCheckCircle } from 'react-icons/hi'
import { Formik, Field, Form } from 'formik'
import { openDialog, updateDialogView, setSelectedBoard, updateColumns, updateOrdered, useAppDispatch, useAppSelector } from '../store'
...
const RenameForm = ({ title, closeRenameForm, columns, ordered }: RenameFormProps) => {
    ...
    return (
        <Formik initialValues={{ title: title }} onSubmit={({ title }) => onFormSubmit(title)}>
                            ...
                            <Field
                                ...
                                suffix={
                                    <div className="flex items-center gap-2">
                                        <HiCheckCircle className="cursor-pointer text-lg text-emerald-500" onClick={submitForm} />
                                    </div>
                                }
                            />
                        </FormItem>
...
const BoardTitle = (props: BoardTitleProps) => {
    ...
    return (
        <div className="board-title px-4 py-3 flex justify-between items-center" {...dragHandleProps}>
            {renameActive ? (
                <>
                    <RenameForm title={title} closeRenameForm={onRenameDeactivate} columns={columns} ordered={ordered} />
                    <HiXCircle className="cursor-pointer text-lg" onClick={onRenameDeactivate} />
                </>
            ) : (
                <>
                    <h6>{title}</h6>
            ...
            >
                <div className="px-6 pb-6 pt-2 flex">
                    ...
                    <div className="ml-4 rtl:mr-4">
                        ...
                        <p>
                            Are you sure you want to delete this board? All the tickets under this board will be deleted as well. This
                            action cannot be undone.
                        </p>
                    </div>
                </div>
                <div className="text-right px-6 py-3 bg-gray-100 dark:bg-gray-700 rounded-bl-lg rounded-br-lg">
                    <Button size="sm" className="ltr:mr-2 rtl:ml-2" onClick={onConfirmDeleteClose}>
                        Cancel
                    </Button>
