...
const ResponsiveText = ({ text, className }: { text: string; className?: string }) => {
    ...
    useEffect(() => {
        const resizeText = () => {
            if (containerRef.current && textRef.current) {
                ...
                while (
                    (textRef.current.offsetWidth > containerWidth || textRef.current.offsetHeight > containerHeight) &&
    ...
    return (
        <div ref={containerRef} className={clsx('w-full h-full overflow-hidden', className)}>
            <p ref={textRef} style={{ fontSize: `${fontSize}px`, lineHeight: 1.2 }} className="break-words">
                {text}
            </p>
        </div>
    )
...
export const GenericBrandCard = ({ component_config, isCarousel, size, data }: BrandCardProps) => {
    const {
        web_name,
        web_name_position,
        web_name_align,
    ...
    const cornerRadiusStyle: React.CSSProperties = web_corner_radius ? { borderRadius: `${web_corner_radius}px` } : {}
