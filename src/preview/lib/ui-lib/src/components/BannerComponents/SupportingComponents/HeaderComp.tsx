...
export function getTextAlignment(position?: AlignmentType): 'flex-start' | 'center' | 'flex-end' {
    switch (position) {
        case 'left':
            return 'flex-start'
        case 'center':
...
export const HeaderWithSubHeader: React.FC<HeaderWithSubHeaderProps> = ({ headerConfig, subHeaderConfig, size }) => {
    const {
        icon: headerIcon = '',
        text: headerText = '',
    ...
    return (
        <div className="w-full flex flex-col">
          ...
          />
                    ...
                    {((size === 'lg' && (webHeaderText || headerText)) || (size !== 'lg' && headerText)) && (
                        <h1
                            className={`${headerTextStyle} `}
                            style={{
                                fontSize: `${size == 'lg' ? headerFontSizeWeb || 24 : headerFontSize || 18}px`,
                                color: headerFontColor || '#A5A4A5',
                            }}
                        >
                ...
                {headerImage && <img className="object-contain w-full" src={headerImage} alt="" />}
            </a>
            <a
          ...
          />
                    ...
                    {/* Render Header Text if available */}
                    {((size === 'lg' && (webSubHeaderText || subHeaderText)) || (size !== 'lg' && subHeaderText)) && (
                        <h1
                            className={`${subHeaderTextStyle}`}
                            style={{
                                fontSize: `${size == 'lg' ? subHeaderFontSizeWeb || 24 : subHeaderFontSize || 18}px`,
                                color: subHeaderFontColor || '#A5A4A5',
                            }}
                        >
                ...
                {subHeaderImage && <img className="object-contain w-full" src={subHeaderImage} alt="" />}
            </a>
        </div>
    )
}
