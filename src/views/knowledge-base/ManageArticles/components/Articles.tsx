...
import { HiChevronRight, HiChevronDown, HiOutlineTrash, HiOutlinePencil, HiOutlineCog, HiOutlinePlus } from 'react-icons/hi'
    ...
    return (
        <div className="mb-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-1 cursor-pointer select-none" onClick={onCollapse}>
                    <span className="text-lg">{collapse ? <HiChevronRight /> : <HiChevronDown />}</span>
                    <h5>{data?.label}</h5>
                    <span>({data?.articles?.length})</span>
                </div>
                <hr className="mx-3 w-full" />
                <Dropdown placement="bottom-end" renderTitle={<Button shape="circle" variant="plain" icon={<HiOutlineCog />} />}>
            ...
            >
                {data.articles.length === 0 && (
                    <Card clickable className="group border-dashed border-2 hover:border-indigo-600" onClick={onArticleAdd}>
...
const Articles = () => {
    ...
    const loading = useAppSelector((state) => state.knowledgeBaseManageArticles.data.loading)
    const categorizedArticles = useAppSelector((state) => state.knowledgeBaseManageArticles.data.categorizedArticles)
