...
export const CouponBanner = ({
  ...
  const mobileContainerClasses = clsx(
    "bg-pink-200 shadow-lg transition-transform duration-500 ease-in-out z-50 ",
    "flex-col"
  );
  ...
  function isTop() {
    return (
      background_config?.desktop_position === "top-right" ||
      background_config?.desktop_position === "top-left"
    );
  }
  function isBottom() {
    return (
      background_config?.desktop_position === "bottom-right" ||
      background_config?.desktop_position === "bottom-left"
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isExpanded ? (
          <FaCaretRight
            size={size == "lg" ? webHeaderFontSize || 50 : headerFontSize || 20}
            color={headerFontColor}
          />
        ) : (
          <FaCaretLeft
