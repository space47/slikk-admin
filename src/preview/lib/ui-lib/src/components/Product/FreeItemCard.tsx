...
export const FreeItemCard = (props: SingleProductProps) => {
    ...
    return (
        <div
            className={`flex flex-col h-fit rounded-xl bg-[#F4F4F4] w-52 ${inventory_count == 0 && 'opacity-80'} ${SizeVariants({ size })}`}
        >
            <a
            ...
            <div className={`flex flex-row justify-between items-start w-full text-[13px] p-1 ${TextVariants({ type })}`}>
                <div className=" flex flex-col w-full">
                    ...
                    <div className="pt-1 text-[16px] flex flex-row items-center justify-between space-x-2 w-full">
                        <div className="flex flex-row items-center justify-center space-x-2 text-sm">
                            {discount > 0 && (
                                <span className=" text-opacity-40 line-through text-[#202020]">₹{Math.round(parseInt(mrp))}</span>
                            )}
                            {parseInt(sp) == 0 ? (
