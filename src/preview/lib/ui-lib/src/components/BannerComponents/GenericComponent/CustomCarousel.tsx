...
export function CustomCarousel<T>({
    ...
    const [emblaRef, emblaApi] = useEmblaCarousel({
        ...
        draggable: !autoplay,
        dragFree: coverFlow ? false : (size == 'sm' && !component_config.carousel) || (size !== 'sm' && !component_config.web_carousel),
    ...
    const toggleAutoplay = useCallback(() => {
        ...
        if (emblaApi) {
            emblaApi.reInit({
                ...
                dragFree: (size == 'sm' && !component_config.carousel) || (size !== 'sm' && !component_config.web_carousel),
                align: coverFlow ? 'center' : 'start',
            })
        }
                ...
                >
                    {items?.map((item, index) => {
                        return (
                            ...
                            >
                                <div
                                    className={coverFlow ? 'transform transition-all duration-300 ease-out py-1' : ''}
                                    style={
                                        coverFlow
                                            ? {
                                                  transform:
