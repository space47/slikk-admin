...
export const GenericComponent = ({
  ...
  const combinedStyle: React.CSSProperties = {
    ...
    paddingLeft:
      size !== "lg"
        ? (component_config?.section_leftPadding ?? 0)
        : (component_config?.web_section_leftPadding ?? "5%"),
    paddingRight:
      size !== "lg"
        ? (component_config?.section_rightPadding ?? 0)
        : (component_config?.web_section_rightPadding ?? "5%"),
    paddingBottom:
      size !== "lg"
        ? (component_config?.section_bottomPadding ?? 0)
        : (component_config?.web_section_bottomPadding ?? 0),
    paddingTop:
      size !== "lg"
        ? (component_config?.section_topPadding ?? 0)
        : (component_config?.web_section_topPadding ?? 0),
    marginLeft:
      size !== "lg"
        ? (component_config?.section_leftMargin ?? 4)
        : (component_config?.web_section_leftMargin ?? 0),
    marginRight:
      size !== "lg"
        ? (component_config?.section_rightMargin ?? 4)
        : (component_config?.web_section_rightMargin ?? 0),
    marginBottom:
      size !== "lg"
        ? (component_config?.section_bottomMargin ?? 0)
        : (component_config?.web_section_bottomMargin ?? 0),
    marginTop:
      size !== "lg"
        ? (component_config?.section_topMargin ?? 20)
        : (component_config?.web_section_topMargin ?? 0),
    borderRadius:
      size !== "lg"
        ? (component_config?.section_corner_radius ?? 0)
        : (component_config?.web_section_corner_radius ?? 0),
    overflow: "hidden",
  };
