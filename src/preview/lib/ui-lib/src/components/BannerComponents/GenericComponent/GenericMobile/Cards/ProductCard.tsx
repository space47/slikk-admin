...
export const GenericProductCardMobile = (props: ProductCardProps) => {
    ...
    const widthStyle: React.CSSProperties = {
        width: isCarousel ? '100%' : `${(component_config?.width || 0.4) * 100}vw`, // Use full width in carousel mode
    }
    ...
    return (
        <div className={`flex flex-col h-fit  ${inventory_count == 0 && 'opacity-80'}  `} style={combinedStyle}>
            ...
            <a
                ...
                style={widthStyle}
            >
                <img src={image} loading="lazy" className={`  object-cover object-top  h-full w-full`} style={cornerRadiusStyle} />
                <div
                    className=" absolute top-0 z-0 left-2 flex justify-center "
                    style={{
            ...
            <div
                className={`flex flex-row justify-between items-start w-full h-[25%] pt-2 text-[13px] ${TextVariants({ type })}`}
                style={{
                    color: footer_font_color || 'black',
                    fontSize: footer_font_size || '13px',
            ...
            >
                <div className=" flex flex-col w-full">
                        ...
                        <a href={fieldsToPDPPage(category, sub_category, name, brand, barcode)}>
                            <span
                                ...
                                style={{
                                    ...
                                    fontSize: footer_font_size || `${calcWidth / 14}vw`,
                                    fontStyle: footer_font_style === 'italic' ? 'italic' : 'normal',
                                    fontWeight: footer_font_style === 'bold' ? '700' : '400',
                                    textDecorationLine: footer_font_style === 'underline' ? 'underline' : 'none',
                                    opacity: 0.5,
                    ...
                    <a href={fieldsToPDPPage(category, sub_category, name, brand, barcode)}>
                        <span
                            className=" line-clamp-1 whitespace-break-spaces capitalize  truncate text-white"
                            style={{
                                // marginTop: `${calcWidth / 40}vw`,
                                marginBottom: `${calcWidth / 40}vw`,
                    ...
                    <div className=" flex flex-row items-center justify-between  w-full">
                        ...
                        >
                            {discount > 0 && <span className=" text-opacity-40 line-through ">₹{Math.round(parseInt(mrp))}</span>}
                            <span
                                style={{
                                    fontSize: `${calcWidth / 11}vw`,
                                }}
                            >
