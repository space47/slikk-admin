...
export default function CountdownTimer({ config, size }: { config: TimerConfig; size: ScreenSize }) {
    ...
    const timerTextStyle = {
        fontSize: size === 'lg' ? `${(config.timer_font_size || 10) * 1.5}px` : `${config.timer_font_size}px`,
        fontWeight: 'semibold',
    ...
    return (
        <div
            className={`w-full ${size === 'lg' ? 'w-11/12' : 'max-w-lg'} mx-auto p-4 sm:p-8 md:rounded-lg`}
            style={{ backgroundColor: config.bg_color || 'black' }}
        >
            <div
                className={`flex items-center  justify-center ${getFlexDirection(config.timer_text_position || 'left')}`}
                style={{ gap: config.timer_gap }}
            >
                <p className={`${getTextAlign(config.timer_text_position || 'left')}  `} style={timerTextStyle}>
                    {config.timer_text}
                </p>
                <div className={`flex `} style={{ gap: 4 }}>
                    {['hours', 'minutes', 'seconds'].map((unit, index) => (
                        <>
                            <div key={unit} className={clsx('text-center mb-4 sm:mb-0', size == 'sm' && 'w-16')}>
                                        ...
                                        .map((digit, i) => (
                                            <div
                                                ...
                                                className={`text-2xl sm:text-3xl ${
                                                    size === 'lg' ? 'lg:text-5xl' : ''
                                                } font-bold w-8 sm:w-10 ${size === 'lg' ? 'lg:w-14' : ''} h-12 sm:h-14 ${
                                                    size === 'lg' ? 'lg:h-20' : ''
                            ...
                            {index < 2 && (
                                <div className="flex items-start justify-center">
                                    <span
                                        className={`text-2xl sm:text-4xl ${size === 'lg' ? 'lg:text-6xl' : ''} font-bold`}
