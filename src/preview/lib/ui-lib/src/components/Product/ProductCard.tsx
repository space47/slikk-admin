...
export const ProductCard = (
  props: SingleProductProps & { showEye?: boolean } & { callbackAct?: () => void }
) => {
  const {
    size,
    type,
    ...
    >
      ...
      >
        <div className="flex flex-col w-full">
          ...
          <a href={fieldsToPDPPage(category, sub_category, name, brand, barcode)}>
            <span className="pt-1 leading-4 line-clamp-2 whitespace-break-spaces capitalize text-opacity-60 h-9 ">
              {name}
            </span>
          </a>
          ...
          <div className="pt-2 text-[16px] flex flex-row items-center justify-between space-x-2 w-full">
            <div
              className={`flex flex-row items-center justify-center space-x-2  ${size == "sm" && "text-[14px]"}`}
            >
              ...
              {discount > 0 && (
                <div className="">
                  <span className={`leading-3 text-[#FF5F15] font-bold mr-1`}>
                    {discount}%
                  </span>
