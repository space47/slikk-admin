...
const GenderContainerVariants = cva('flex flex-row  rounded-xl relative border bg-gradient-to-l cursor-pointer', {
    variants: {
        size: {
...
export const GenderSelector = ({
    ...
    const [selectedOption, setSelectedOption] = useState<GenderData>(option.filter((g) => g?.value == defVal)[0])
    ...
    return (
        <div
            className={clsx(ComponentVariants({ type: genderPosition || 'centered' }), size == 'sm' ? ' items-end' : ' items-center h-20')}
        >
            {option?.map((data, key) => {
                const isSelected = selectedOption?.value == data?.value
                return (
                    <div className={clsx('flex justify-center items-end', size == 'sm' ? 'h-12' : 'h-16')} key={key}>
                        <div
                            //
                            className={clsx(
                        ...
                        >
                            ...
                            <div className={'flex items-center justify-center h-full w-full'}>
                                <p className={clsx('pl-2 text-white', isSelected && 'font-semibold')}>{data.name}</p>
                            </div>
                        </div>
                    </div>
                )
