...
import { getLogs, filterLogs, setActivityIndex, useAppDispatch, useAppSelector } from '../store'
...
const Log = () => {
    ...
    const loadMoreLoading = useAppSelector((state) => state.accountActivityLog.data.loadMoreLoading)
    const loadable = useAppSelector((state) => state.accountActivityLog.data.loadable)
            ...
            <div className="max-w-[900px]">
                {logs.map((log) => (
                    <div key={log.id} className="mb-8">
                        <div className="mb-4 font-semibold uppercase">{dayjs.unix(log.date).format('dddd, DD MMMM')}</div>
                        <Timeline>
                            {isEmpty(log.events) ? (
