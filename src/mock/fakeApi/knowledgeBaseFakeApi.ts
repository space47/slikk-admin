...
export default function knowledgeBaseFakeApi(server: Server, apiPrefix: string) {
    const categories: {
        label: string
        value: string
        articles?: unknown[]
    ...
    server.get(`${apiPrefix}/knowledge-base/others-article`, (schema, { queryParams }) => {
        ...
        const relatedArticle = sameCategoryArticle.filter((article: { id: string }) => article.id !== id)
        // eslint-disable-next-line  @typescript-eslint/no-explicit-any
        articles = articles.filter((article) => article.id !== id) as any
        const popularArticle = [articles[0], articles[4], articles[7], articles[11]]
        return {
            relatedArticle,
            popularArticle,
        }
