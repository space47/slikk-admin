...
const Billing = () => {
    ...
    const fetchData = async () => {
        const response = await apiGetAccountSettingBillingData<GetAccountSettingBillingDataResponse>()
        setData(response.data)
    }

    const onFormSubmit = (_: BillingFormModel, setSubmitting: (isSubmitting: boolean) => void) => {
        toast.push(<Notification title={'Billing information updated'} type="success" />, {
    ...
    const onEditCreditCard = (card: Partial<CreditCard>, type: 'EDIT' | 'NEW') => {
        setCcDialogType(type)
        setSelectedCard(card)
    }

    const onCardUpdate = (cardValue: CreditCardInfo, form: FormikProps<BillingData>, field: FieldInputProps<BillingData>) => {
        let paymentMethodsValue = form.values[field.name as keyof BillingData] as CreditCard[]
        ...
        >
            {({ values, touched, errors, isSubmitting, resetForm }) => {
                ...
                return (
                    <Form>
                        <FormContainer>
                            <FormDesription title="Payment Method" desc="You can update your cards information here" />
                                ...
                                <div className="rounded-lg border border-gray-200 dark:border-gray-600">
                                    {values?.paymentMethods?.map((card, index) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex">
                                                <Button size="sm" type="button" onClick={() => onEditCreditCard(card, 'EDIT')}>
                                                    Edit
                                                </Button>
                                            </div>
                                        </div>
                                    ...
                                    {values?.otherMethod?.map((method, index) => (
                                        ...
                                        >
                                            <div className="flex items-center">
                                                ...
                                                <div className="ml-3 rtl:mr-3 font-semibold">{method.identifier}</div>
                                            </div>
                                            <div className="flex">
                                                <Button size="sm" type="button" onClick={() => onRedirect(method.redirect)}>
                                                    Edit
                                                </Button>
                                            </div>
                                        </div>
                            ...
                            <FormDesription className="mt-6" title="Billing History" desc="View your previos billing" />
                            <BillingHistory
